...
class CcPopUpAdmin {
   ...
   * 
   */
  function chch_pu_render_newsletter_select( $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
  ...
  function chch_pu_save_pop_up_meta( $post_id, $post, $update ) {
    if ( !isset( $_POST['chch_pu_save_nonce'] ) || !wp_verify_nonce( $_POST['chch_pu_save_nonce'], 'chch_pu_save_nonce_' . $post_id ) ) {
      return;
    }
    ...
    if ( !empty( $template ) ) {
      ...
      $template_data['input'] = array( 'text' => sanitize_text_field( $_REQUEST['_' . $template . '_input_text'] ), );

      $template_data['button'] = array( 'text' => sanitize_text_field( $_REQUEST['_' . $template . '_button_text'] ), );
  ...
  public function chch_pu_enqueue_admin_scripts() {
    ...
    if ( 'post' == $screen->base && 'chch-pop-up' === $screen->post_type ) {
      ...
      wp_enqueue_style( $this->plugin_slug . '-ui', '//ajax.googleapis.com/ajax/libs/jqueryui/1.10.3/themes/smoothness/jquery-ui.min.css', null, CcPopUp::VERSION, 'all' );
