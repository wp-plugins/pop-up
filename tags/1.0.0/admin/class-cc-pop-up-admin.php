...
if ( ! class_exists( 'CcPopUpPeview' ) )
    require_once( dirname( __FILE__ ) . '/includes/cc-pop-up-preview.php' );
...
class CcPopUpAdmin { 
	...
	function cc_pu_tinymce_event($plugin_array) { 
		$plugin_array['keyup_event'] = CC_PU_PLUGIN_URL .'admin/assets/js/cc-tinymce.js';
	...
	function custom_pop_up_columns($defaults) {
		$defaults['cc_pu_status'] = __('Active');
		$defaults['cc_pu_clicks'] = __('Clicks');
		$defaults['cc_pu_template'] = __('Template');
		return $defaults;
	}
	...
	function manage_custom_pop_up_columns($column, $post_id) {
		...
		if ($column === 'cc_pu_status') {
			echo ucfirst(get_post_meta($post_id,'_cc_pop_up_status', true));
		}
		...
		if ($column === 'cc_pu_template') {
			echo ucfirst(get_post_meta($post_id,'_cc_pop_up_template', true));
		}
	}
	...
	public function register_cc_pu_post_type() {
		...
		$labels = array(
			'name'                => _x( 'PopUps', 'Post Type General Name', $domain),
			'singular_name'       => _x( 'PopUp', 'Post Type Singular Name', $domain),
			'menu_name'           => __( 'PopUp', $domain),
			'parent_item_colon'   => __( 'Parent Item:', $domain),
			'all_items'           => __( 'PopUps', $domain),
			'view_item'           => __( 'View Item', $domain),
			'add_new_item'        => __( 'Add New PopUp', $domain),
			'add_new'             => __( 'Add New', $domain),
			'edit_item'           => __( 'Edit PopUp', $domain),
			'update_item'         => __( 'Update PopUp', $domain),
			'search_items'        => __( 'Search PopUp', $domain),
			'not_found'           => __( 'Not found', $domain),
			'not_found_in_trash'  => __( 'No PopUp found in Trash', $domain),
		);
	...
	function save_pop_up_meta( $post_id, $post, $update ) { 
		if (
			!isset($_POST['cc_pu_save_nonce']) 
			|| ! wp_verify_nonce($_POST['cc_pu_save_nonce'],'cc_pu_save_nonce_'.$post_id) 
		) 
		{
			return;
		}
		...
		$template =  $_REQUEST['_cc_pop_up_template'];
		update_post_meta( $post_id, '_cc_pop_up_template', sanitize_text_field( $_REQUEST['_cc_pop_up_template']) );
		update_post_meta( $post_id, '_cc_pop_up_base', sanitize_text_field( $_REQUEST['_cc_pop_up_template_base']) );
	...
	public function enqueue_cc_pu_admin_scripts() {
		...
		if ( 'post' == $screen->base && 'chch-pop-up' === $screen->post_type) { 
			...
			wp_register_script($this->plugin_slug .'-upload-js',  plugins_url( 'assets/js/upload.js', __FILE__ ), array('jquery'), CcPopUp::VERSION ); 
			wp_enqueue_script($this->plugin_slug .'-upload-js');
			
			wp_enqueue_style( $this->plugin_slug .'-tabs', plugins_url( 'assets/css/cc-tabs.css', __FILE__ ), array(), CcPopUp::VERSION );
			wp_enqueue_script( $this->plugin_slug .'-tabs-js', plugins_url( 'assets/js/cc-tabs.js', __FILE__ ), array( 'jquery', 'wp-color-picker' ), CcPopUp::VERSION ); 
			
			wp_register_script($this->plugin_slug .'-admin-js',  plugins_url( 'assets/js/cc-pu-admin.js', __FILE__ ), array('jquery', 'wp-color-picker'), CcPopUp::VERSION );
			wp_enqueue_script($this->plugin_slug .'-admin-js');
			wp_localize_script( $this->plugin_slug .'-admin-js', 'ajax_object', array( 'ajaxUrl' => admin_url( 'admin-ajax.php' )) );
