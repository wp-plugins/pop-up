...
class CcPopUp {
	...
	private function __construct() {
		...
		//ladd_action( 'init', array( $this, 'load_plugin_textdomain' ) );
	...
	function template_scripts() { 
		...
		if(!empty($pop_ups)) {
			...
			if(file_exists(CC_PU_PLUGIN_DIR . 'public/assets/js/ajax.js')){	
				wp_enqueue_script( $this->plugin_slug .'ajax-script', CC_PU_PLUGIN_URL . 'public/assets/js/ajax.js', array('jquery') ); 
				wp_localize_script( $this->plugin_slug .'ajax-script', 'ajax_object', array( 'ajaxUrl' => admin_url( 'admin-ajax.php' )) );
			}
			...
			if(file_exists(CC_PU_PLUGIN_DIR . 'public/assets/js/ajax.js')){	
				wp_enqueue_script( $this->plugin_slug .'ajax-script', CC_PU_PLUGIN_URL . 'public/assets/js/ajax.js', array('jquery', $this->plugin_slug .'jquery-cookie') ); 
				wp_localize_script( $this->plugin_slug .'ajax-script', 'ajax_object', array( 'ajaxUrl' => admin_url( 'admin-ajax.php' )) );
			}
			...
			foreach($pop_ups as $id)
			{
				...
				$template_id = get_post_meta( $id, '_cc_pop_up_template', true);
				$template_base = get_post_meta( $id, '_cc_pop_up_base', true);
	 ...
	 public function show_popup() {
		...
		if(!empty($pop_ups))
		{
			...
			{ 
				...
				$user_role = get_post_meta( $id, '_cc_pop_up_role', true);
				...
				$pages = get_post_meta( $id, '_cc_pop_up_page', true);
				if(is_array( $pages)){
	...
	public function ajax_newsletter_subscribe() {
		...
		if(!check_ajax_referer( 'cc-pu-newsletter-subscribe' , 'nounce')){
			print json_encode(array('status' => 'cheating'));
			die();	
		}
		
		$id = $_POST['popup'];
		$to_email = get_post_meta( $id, '_cc_pop_up_email', true) ? get_post_meta( $id, '_cc_pop_up_email', true) : get_option( 'admin_email' );
